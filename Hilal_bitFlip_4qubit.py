...
def simulate_povm(qc, povm_elements, simulator, shots):
    ...
    # 2. Bit-flip gürültüsü oluştur
    p_bit = 0.8  # Bit-flip olasılığı
    bit_flip_noise = pauli_error([('X', p_bit), ('I', 1 - p_bit)])  # Bit-flip hatası

    # Gürültü modelini tanımla ve Bob'a giden qubit'e uygula
    noise_model_bit = NoiseModel()
    noise_model_bit.add_quantum_error(bit_flip_noise, ['id', 'u3'], [1])  # 5. qubit (Bob'a giden qubit)
    noise_model_bit.add_quantum_error(bit_flip_noise, ['id', 'u3'], [3])  # 7. qubit (Bob'a giden qubit)
    


    esult=transpile(qc, simulator)
    job = simulator.run(qc, noise_model=noise_model_bit, shots=1024)  # Fazladan simulator argümanı kaldırıldı
...
Channel_BitFlip_x=np.vstack([probabilities_z, probabilities_x , probabilities_y ])
Channel_BitFlip = np.sum(Channel_BitFlip_x , axis=0)
Probability_BitFlip=np.mean(Channel_BitFlip )
print(Probability_BitFlip)
