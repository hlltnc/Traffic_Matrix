...
# Create a circuit that generates 4 Bell states
def create_bell_state_circuit():
    qc = QuantumCircuit(4, 4)  # 8 qubits, 8 classical bits
...
def simulate_povm(qc, povm_elements, simulator, shots):
    ...
    p_dep = 0.1  # Depolarizasyon olasılığı
    depolarizing_noise = depolarizing_error(p_dep, 1)  # İlk parametre olasılık, ikinci parametre qubit sayısı


    noise_model_dep = NoiseModel()
    noise_model_dep.add_quantum_error(depolarizing_noise, ['id', 'u3'], [1])  # İkinci qubit (Bob'a giden qubit)
    noise_model_dep.add_quantum_error(depolarizing_noise, ['id', 'u3'], [3])  # dorduncu qubit (Bob'a giden qubit)



    esult=transpile(qc, simulator)
    job = simulator.run(qc, noise_model=noise_model_dep, shots=1024)  # Fazladan simulator argümanı kaldırıldı
    ...
    povm_indices = np.arange(256)  # Range for 4^8 possibilities
    ...
    for idx in povm_indices:  # Loop through all 256 possible outcome indices
        ...
        for outcome, count in counts.items():
            outcome_state = np.array([[1, 0], [0, 0]]) if outcome == '0' else np.array([[0, 0], [0, 1]])
            for q in range(1, 4):
...
# Create the Bell state circuit with 4 Bell states (8 qubits total)
...
plt.title('POVM Measurement Probabilities in Various Bases depolarization')
plt.legend()
plt.grid(True)
plt.show()
...
Channel_Depolar_x=np.vstack([probabilities_z, probabilities_x , probabilities_y ])
Channel_Depolar = np.sum(Channel_Depolar_x , axis=0)
Probability_Depolar=np.mean(Channel_Depolar )
print(Probability_Depolar)
