...
def add_depolarizing_noise():
    ...
    # Create a bit-flip error as a Pauli-X operation with a certain probability
    error_1q = pauli_error([('X', 0.5), ('I', 0.5)])  # 10% chance of a bit flip, 99% chance no error
    error_2q = pauli_error([('XX', 0.5), ('II', 0.5)])  # 20% chance of bit flip on both qubits

    # Add errors to single-qubit gates
    noise_model.add_all_qubit_quantum_error(error_1q, ['h', 'sdg'])
...
def perform_tomography(circuits, shots=1024):
    ...
    job = simulator.run(circuits, noise_model=noise_model, shots=shots)
