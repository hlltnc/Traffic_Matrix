...
bitflip_correct_prob_list=[]
depolar_correct_prob_list=[]
...
for i in range(1000):
    ...
    if predicted_channel == chosen_channel:
        ...
        if chosen_channel == BitFlip_channel:
            ...
            bitflip_correct_prob=bitflip_correct_count/1000
            bitflip_correct_prob_list.append(bitflip_correct_prob)
        else:
            depolar_correct_count += 1
            depolar_correct_prob=depolar_correct_count/1000
            depolar_correct_prob_list.append(depolar_correct_prob)
...
print(f"BitFlip Capacities: {len(bitflip_capacities)}")
print(f"Depolar Capacities: {len(depolar_capacities)}")



print(f"BitFlip probabilities: {bitflip_correct_prob_list}")
print(f"Depolar probabilities: {depolar_correct_prob_list}")
print(f"BitFlip probabilities: {len(bitflip_correct_prob_list)}")
print(f"Depolar probabilities: {len(depolar_correct_prob_list)}")


# Plot for Channel Capacities vs Success Probability
#figure(1)
#plt.figure(figsize=(10, 5))

# Plot for BitFlip channel
#plt.subplot(1, 2, 1)
#plt.scatter(np.sort(bitflip_capacities), np.sort(bitflip_correct_prob_list), color='blue', alpha=0.7)
#plt.title('BitFlip Channel: Capacity vs Success Probability')
#plt.xlabel('Channel Capacity (BitFlip)')
#plt.ylabel('Success Probability (BitFlip)')
#plt.grid(True)

# Plot for Depolarizing channel
#plt.subplot(1, 2, 2)
#plt.scatter(np.sort(depolar_capacities),np.sort(depolar_correct_prob_list) , color='green', alpha=0.7)
#plt.title('Depolarizing Channel: Capacity vs Success Probability')
#plt.xlabel('Channel Capacity (Depolarizing)')
#plt.ylabel('Success Probability (Depolarizing)')
#plt.grid(True)
...
# BitFlip veri seti
plt.scatter(np.sort(bitflip_capacities), np.sort(bitflip_correct_prob_list), color='blue', label='BitFlip Channel', alpha=0.7)

# Depolarizing veri seti
plt.scatter(np.sort(depolar_capacities), np.sort(depolar_correct_prob_list), color='green', label='Depolarizing Channel', alpha=0.7)

# Grafik ayarları
#plt.title('Channel Capacities vs Success Probabilities')
