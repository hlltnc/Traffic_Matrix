...
def add_bit_flip_noise():
    ...
    error_1q = pauli_error([('X', 0.4), ('I', 0.6)])  # 5% chance of bit-flip, 95% no error
    noise_model.add_all_qubit_quantum_error(error_1q, ['h', 'x'])
    error_2q = pauli_error([('XX', 0.4), ('II', 0.6)])  # 5% chance of bit-flip on both qubits
...
def plot_running_time(max_qubits=6, shots=8192):
   ...
   # plt.title("Running Time vs. Number of Qubits")
    plt.legend()
    plt.grid(True)
    plt.savefig('running_time_plot.pdf')
    plt.show()
