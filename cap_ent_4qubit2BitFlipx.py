...
def get_full_density_matrix():
    ...
    import State_Tomog_4qubit_BitFlip
    import Tomog_NoNoise

    circuits = State_Tomog_4qubit_BitFlip.generate_tomography_circuits()
    counts = State_Tomog_4qubit_BitFlip.perform_tomography(circuits, shots=1024)
    rho = State_Tomog_4qubit_BitFlip.reconstruct_density_matrix(counts, shots=1024)
...
#print(f"Quantum channel capacity list: C(Φy) = {Channel_Capacity_list_bitFlip}")


# Plot number of uses vs. channel capacity
#plt.plot(numb_of_uses_list, Channel_Capacity_list_bitFlip, marker='o', linestyle='-', color='b')
